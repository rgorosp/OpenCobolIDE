       ...
       PROGRAM-ID. C4A02P05.
      *---------------------------------------------------------------
      * PROGRAMA: CAPITULO04 - AULA 02 - PRATICA 05
      *---------------------------------------------------------------
      *   LPAR     | V | DESCRICAO                             | DATA
      *   PRE7     |001| MONTAGEM DO PROGRAMA C4A02P05         | 25/11
      * ========== |===| ===================================== | =====
      ...
      *-------------------------------------------------------------*
       ...
       FILE-CONTROL.
           ...
           .
           SELECT C4P02RE1 ASSIGN TO "C:/ARQUIVOS/R1-C4A02P05.txt"
      ...
      *==> AREA DESTINADA A CONTADORES E ACUMULADORES
       01  WS-AREA-AUX.
           ...
           05 WS-I-CONS              PIC 9(02) VALUE ZEROS.
           05 WS-O-RE1               PIC 9(11) VALUE ZEROS.
           05 WS-FUNC                PIC 9(11) VALUE ZEROS.
           05 WS-CODIGO              PIC 9     VALUE ZEROS.
      ...
      *==> AREA DESTINADA A RODAPE.
       01  WS-ROD1.
           ...
           05 FILLER                 PIC X(09).
           05 TOT-FUNC-ROD1          PIC 9(02).
           05 FILLER                 PIC X(07).

       01  WS-ROD2.
           05 FILLER                 PIC X(28).
           05 FILLER                 PIC X(35) VALUE
              'TOTAL DE SALARIOS NO DEPARTAMENTO: '.
           05 TOT-SALARIO-ROD2       PIC $ZZ.ZZ9.99.
           05 FILLER                 PIC X(07).
      ...
      *--------------------------------------------------------------*
      ...
      D    READY TRACE
       ...
       000-C4A02P05-FIM.
           EXIT.
      *--------------------------------------------------------------*
      *  PROCEDIMENTO - ENTRADA VIA CONSOLE
      *--------------------------------------------------------------*
       001-CONSOLE SECTION.
       001-CONSOLE-INI.
           DISPLAY "Digite o numero do Departamento" UPON CONSOLE
           ACCEPT WS-I-CONS FROM CONSOLE
           IF WS-I-CONS = '99' THEN
              PERFORM 100-TERMINO
           ELSE
              PERFORM 002-LEITURA-CONS
           END-IF
           .
       001-CONSOLE-FIM.
           EXIT.
      ...
      *--------------------------------------------------------------*
       ...
       015-ABRIR-FIM.
           EXIT.
      *--------------------------------------------------------------*
      *   CHECAGEM DA MATRICULA INFORMADA NA CONSOLE
      *--------------------------------------------------------------*
       002-LEITURA-CONS SECTION.
       002-LEITURA-CONS-INI.
           MOVE WS-I-CONS TO KEY-EN1 WS-DEPTO

           START C4P02EN1 KEY EQUAL KEY-EN1
           DISPLAY 'FILE STATUS DO START = ' WS-FS-EN1
           DISPLAY 'CODIGO DA CHAVE      = ' KEY-EN1

           IF WS-FS-EN1 NOT EQUAL '00'
              MOVE 'ERRO NO START DO ARQUIVO C4P02EN1'    TO WS-MSG-A
              MOVE WS-FS-EN1                              TO WS-MSG-B
           PERFORM 100-TERMINO
           GO TO 999-ERRO
           ELSE
      ...
      *--------------------------------------------------------------*
      ...
      D    EXHIBIT NAMED WS-REG-C4P02EN2
           ...
           IF DEPTO-EN2 NOT = WS-DEPTO AND WS-FLAG >= 1
              GO TO 054-RODAPE-INI
           END-IF

           IF DEPTO-EN2 NOT = WS-DEPTO
              ADD 1 TO WS-I-EN2
              GO TO 020-LEITURA-INI
           END-IF
           ...
           ELSE
              ...
              ELSE
                 IF WS-FS-EN2 EQUAL '00'
                    ADD 1 TO WS-I-EN2 WS-FLAG
                    PERFORM 050-PROCESSO
                 END-IF
              END-IF
           END-IF
           .
      ...
      *--------------------------------------------------------------*
       ...
       051-IMPREL-INI.
           ...
           IF DEPTO-EN2 = WS-DEPTO AND WS-FLAG = 1
              PERFORM 052-CAB3
           END-IF
      ...
      *--------------------------------------------------------------*
       ...
       054-RODAPE-INI.
           WRITE REG-C4P02RE1 FROM WS-SPACES
           IF WS-FS-RE1 NOT = '00'
              MOVE 'ERRO NA GRAVACAO DO SPACE6'   TO WS-MSG-A
              MOVE WS-FS-RE1                      TO WS-MSG-B
           GO TO 999-ERRO
           END-IF

           MOVE WS-FUNC TO TOT-FUNC-ROD1
           ...
           MOVE WS-TOT-CALC TO TOT-SALARIO-ROD2
           WRITE REG-C4P02RE1 FROM WS-ROD2
           IF WS-FS-RE1 NOT = '00'
              MOVE 'ERRO NA GRAVACAO DO RODAPE2'   TO WS-MSG-A
              MOVE WS-FS-RE1                       TO WS-MSG-B
           GO TO 999-ERRO
           END-IF

           MOVE ZEROS TO WS-FLAG WS-FUNC WS-TOT-CALC
           CLOSE C4P02EN2
           OPEN  INPUT C4P02EN2
           PERFORM 001-CONSOLE-INI
           .
       054-RODAPE-FIM.
           EXIT.
      *--------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       ...
       998-VAZIO-INI.
           ...
           DISPLAY '     C4A02P05 SEM MOVIMENTO, FV. VERIFICAR.   '
           DISPLAY '=============================================='
      ...
      *--------------------------------------------------------------*
       ...
       999-ERRO-INI.
           DISPLAY '============================================'
           DISPLAY '        ERRO NO PROGRAMA C4A02P05           '
           DISPLAY '============================================'
      ...
      *--------------------------------------------------------------*
       ...
       9991-ERRO-INI.
           DISPLAY '============================================'
           DISPLAY '        DISPLAY NO PROGRAMA C4A02P05           '
           DISPLAY '============================================'
           DISPLAY 'MENSAGEM = ' WS-MSG-A
           DISPLAY 'CODIGO   = ' WS-MSG-B
