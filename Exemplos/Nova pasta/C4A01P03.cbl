       ...
       PROGRAM-ID. C4A01P03.
      *---------------------------------------------------------------
      * PROGRAMA: CAPITULO 04 - AULA 01 - PRATICA 03
      *---------------------------------------------------------------
      * ========== |===| ===================================== | =====
      *   DDNAME   |I/O|          DESCRICAO         |   COPY   | LRECL
      * ========== |===| ========================== | ======== | =====
      * - C4A01PE1 | I | ARQUIVO C4A01PE1 SEQUENCIAL|  - - - - |  040
      * - C4A01PE2 |I-O| ARQUIVO C4A01PE2 RELATIVO  |  - - - - |  040
      * - C4A01PR1 | O | ARQUIVO C4A01PR1 RELATORIO |  - - - - |  080
      *---------------------------------------------------------------
      ...
      *-------------------------------------------------------------*
       ...
       FILE-CONTROL.
           SELECT C4A01PE1 ASSIGN TO "C:/Arquivos/E1-C4A01P03.TXT"
           ...
           .
           SELECT C4A01PR1 ASSIGN TO "C:/Arquivos/R1-C4A01PR1.TXT"
                  ORGANIZATION IS LINE SEQUENTIAL
      ...
      *==> AREA DESTINADA A RODAPE.
       01  WS-ROD2.
           ...
           05 FILLER                 PIC X(25) VALUE
              'REGISTROS NAO EXCLUIDOS: '.
           05 TOT-REG-NAO-EXCLU      PIC 9(05).
           05 FILLER                 PIC X(09).
      ...
      *--------------------------------------------------------------*
       ...
       015-ABRIR-INI.
           ...
           IF WS-FS-PE1 NOT EQUAL '00'
              MOVE 'ERRO AO ABRIR O ARQUIVO C4A01PE1'    TO WS-MSG-A
           ...
           OPEN OUTPUT C4A01PR1
           IF WS-FS-PR1 NOT EQUAL '00'
              MOVE 'ERRO AO ABRIR O ARQUIVO C4A01PR1'    TO WS-MSG-A
      ...
      *--------------------------------------------------------------*
       020-LEITURA SECTION.
       020-LEITURA-INI.
           READ C4A01PE1 INTO WS-REG-C4A01PE1

      D    EXHIBIT NAMED WS-REG-C4A01PE1

           IF WS-FS-PE1 NOT EQUAL '00' AND '10'
              MOVE 'ERRO NA LEITURA DO ARQUIVO C4A01PE1'  TO WS-MSG-A
              MOVE WS-FS-PE1                              TO WS-MSG-B
           GO TO 999-ERRO
           ELSE
              IF WS-FS-PE1 EQUAL '10' AND WS-I-PE1 = 0
                 MOVE 'ARQUIVO C4A01PE1 SEM MOVIMENTO'    TO WS-MSG-A
                 MOVE WS-FS-PE1                           TO WS-MSG-B
              GO TO 998-VAZIO
              ELSE
      ...
      *    READ C4A01PE2 INTO WS-REG-C4A01PE2
           DELETE C4A01PE2
           IF WS-FS-PE2 = '23'
               ADD 1 TO TOT-REG-NAO-EXCLU
               DISPLAY '*-------------------------------*'
               MOVE 'REGIS NAO EXCLUIDO' TO OCORRENCIA-PR1
               DISPLAY '*-------------------------------*'
               DISPLAY 'ERRO NO DELETE DO ARQUIVO C4A01PE2'
               DISPLAY 'CHAVE NAO DELETADA: ' KEY-PE2
               DISPLAY 'PROCESSO NORMAL.'
               DISPLAY '*-------------------------------*'
           ELSE
           ...
           ELSE
           IF WS-FS-PE2 = '00'
               ADD 1 TO WS-DEL-PE2 TOT-REG-EXCLU
               MOVE 'REGISTRO EXCLUIDO' TO OCORRENCIA-PR1
               DISPLAY '*-------------------------------*'
               DISPLAY 'CHAVE ' KEY-PE2 ' DELETADA COM SUCESSO'
               DISPLAY '*-------------------------------*'
           END-IF
           END-IF
           END-IF
      ...
      *--------------------------------------------------------------*
       ...
       051-IMPREL-INI.
           ...
           WRITE REG-C4A01PR1 FROM WS-DET1
           IF WS-FS-PR1 NOT = '00'
              MOVE 'ERRO NA GRAVACAO DO C4A01PR1 DET1'  TO WS-MSG-A
              MOVE WS-FS-PR1                            TO WS-MSG-B
           GO TO 999-ERRO
           ELSE
              ADD 1                                     TO WS-O-PR1
      ...
      *--------------------------------------------------------------*
       052-IMPCAB SECTION.
       052-IMPCAB-INI.
           WRITE REG-C4A01PR1 FROM WS-HIFEN
           IF WS-FS-PR1 NOT = '00'
           ...
           WRITE REG-C4A01PR1 FROM WS-CAB1
           IF WS-FS-PR1 NOT = '00'
           ...
           WRITE REG-C4A01PR1 FROM WS-CAB2
           IF WS-FS-PR1 NOT = '00'
           ...
           WRITE REG-C4A01PR1 FROM WS-SPACES
           IF WS-FS-PR1 NOT = '00'
           ...
           WRITE REG-C4A01PR1 FROM WS-CAB3
           IF WS-FS-PR1 NOT = '00'
      ...
      *--------------------------------------------------------------*
       ...
       053-IMPROD-INI.
           ...
           WRITE REG-C4A01PR1 FROM WS-ROD1
           IF WS-FS-PR1 NOT = '00'
           ...
           WRITE REG-C4A01PR1 FROM WS-ROD2
           IF WS-FS-PR1 NOT = '00'
      ...
      *--------------------------------------------------------------*
       ...
       100-TERMINO-INI.
           ...
           DISPLAY ' '.
           DISPLAY 'REG.LIDOS   C4A01PE1 = ' WS-I-PE1
           DISPLAY 'REG.DELETE  C4A01PE2 = ' WS-DEL-PE2
           DISPLAY 'REG.GRAVADO C4A01PR1 = ' WS-O-PR1
           DISPLAY '======================================='
      ...
      *--------------------------------------------------------------*
       ...
       080-FECHAR-INI.
           CLOSE C4A01PE1
           IF WS-FS-PE1 NOT EQUAL '00'
              MOVE 'ERRO NO CLOSE DO ARQUIVO C4A01PE1'    TO WS-MSG-A
              MOVE WS-FS-PE1                              TO WS-MSG-B
           GO TO 999-ERRO
           END-IF
           ...
           CLOSE C4A01PR1
           IF WS-FS-PR1 NOT EQUAL '00'
              MOVE 'ERRO NO CLOSE DO ARQUIVO C4A01PR1'    TO WS-MSG-A
              MOVE WS-FS-PR1                              TO WS-MSG-B
           GO TO 999-ERRO
           END-IF
           .
