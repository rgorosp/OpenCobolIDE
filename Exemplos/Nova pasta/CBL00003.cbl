       ...
       PROGRAM-ID. C3A07P01.
      *---------------------------------------------------------------
      * PROGRAMA: CAPITULO 03 - AULA 07 - PRATICA01
      *---------------------------------------------------------------
      * ========== |===| ===================================== | =====
      *   DDNAME   |I/O|          DESCRICAO         |   COPY   | LRECL
      * ========== |===| ========================== | ======== | =====
      * - ENTRADA  | I | ARQUIVO ENTRADA SEQUENCIAL |  - - - - |  034
      * - SAIDA    | O | ARQUIVO SAIDA SEQUENCIAL   |  - - - - |  023
      *---------------------------------------------------------------
      ...
      *-------------------------------------------------------------*
       ...
       FILE-CONTROL.
           SELECT ENTRADA
                  ASSIGN TO 'C:\Arquivos\E1-C3A07P01.txt'
                  ORGANIZATION IS LINE SEQUENTIAL
                  FILE STATUS IS WS-FS-E01
           .
           SELECT SAIDA    ASSIGN
                  TO 'C:\Arquivos\S1-C3A07P01.txt'
                  ORGANIZATION IS LINE SEQUENTIAL
      ...
      *--------------------------------------------------------------*
       ...
       FD  ENTRADA
           RECORDING MODE IS F.
       01  REG-ENTRADA               PIC X(034).

       FD  SAIDA
           RECORDING MODE IS F.
       01  REG-SAIDA                 PIC X(023).
      *--------------------------------------------------------------*
      ...
      *==> AREA DESTINADA A CONTADORES E ACUMULADORES
       01  WS-AREA-AUX.
           ...
           05 WS-SALBRU              PIC 9(07)V99 VALUE ZEROS.
      ...
      *==> AREA DESTINADA A ENTRADA.
       01  WS-REG-ENTRADA.
           05 MATRICULA-E01          PIC 9(04).
           05 VALOR-HORA-E01         PIC 999V99.
           05 HORAS-MES-E01          PIC 9(05).
           05 FUNCIONARIO-E01        PIC X(20).
      *==> AREA DESTINADA A SAIDA.
       01  WS-REG-SAIDA.
           05 MATRICULA-S01          PIC 9(04).
           05 VALOR-HORA-S01         PIC 999V99.
           05 HORAS-MES-S01          PIC 9(05).
           05 ADIANTAMENTO-S01       PIC 9(07)V99.
      ...
      *--------------------------------------------------------------*
       ...
       050-PROCESSO-INI.
           ...
           COMPUTE WS-SALBRU = (VALOR-HORA-E01 * HORAS-MES-E01) * 0,40

      D    EXHIBIT NAMED WS-SALBRU

           MOVE MATRICULA-E01  TO MATRICULA-S01
           MOVE VALOR-HORA-E01 TO VALOR-HORA-S01
           MOVE HORAS-MES-E01  TO HORAS-MES-S01
           MOVE WS-SALBRU      TO ADIANTAMENTO-S01
       ...
       050-PROCESSO-FIM.
           EXIT.
      *--------------------------------------------------------------*
      *  PROCEDIMENTO - TERMINO DO PROCESSAMENTO DO PROGRAMA C3A07P01
      *--------------------------------------------------------------*
       100-TERMINO SECTION.
       100-TERMINO-INI.
           PERFORM 010-DATA-HORA-DAY
           PERFORM 080-FECHAR
           DISPLAY '============= PGM.C3A07P01 ============'
           DISPLAY ' '.
      ...
      *----------------------------------------------------------------*
       ...
       998-VAZIO-INI.
           ...
           DISPLAY '     C3A07P01 SEM MOVIMENTO, FV. VERIFICAR.   '
           DISPLAY '=============================================='
      ...
      *--------------------------------------------------------------*
       ...
       999-ERRO-INI.
           DISPLAY '============================================'
           DISPLAY '        ERRO NO PROGRAMA C3A07P01           '
           DISPLAY '============================================'
